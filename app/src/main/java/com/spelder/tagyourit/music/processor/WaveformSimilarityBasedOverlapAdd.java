...
import com.spelder.tagyourit.music.player.TrackWriter;

/**
 * An overlap-add technique based on waveform similarity (WSOLA) for high quality time-scale
 * modification of speech
 *
 * <p>A concept of waveform similarity for tackling the problem of time-scale modification of speech
 * is proposed. It is worked out in the context of short-time Fourier transform representations. The
 * resulting WSOLA (waveform-similarity-based synchronized overlap-add) algorithm produces
 * high-quality speech output, is algorithmically and computationally efficient and robust, and
 * allows for online processing with arbitrary time-scaling factors that may be specified in a
 * time-varying fashion and can be chosen over a wide continuous range of values.
 *
 * <p>Inspired by the work soundtouch by Olli Parviainen, http://www.surina.net/soundtouch,
 * especially the TDStrech.cpp file.
 *
 * @author Joren Six
...
public class WaveformSimilarityBasedOverlapAdd implements AudioProcessor {
  ...
  private void applyNewParameters() {
    ...
    int highestOneBit = Integer.highestOneBit(sampleReq);
    if (sampleReq != highestOneBit) {
      sampleReq = highestOneBit << 1;
    }
  ...
  private void overlap(final float[] output, int outputOffset, float[] input, int inputOffset) {
    for (int i = 0; i < overlapLength; i++) {
      ...
      output[i + outputOffset] =
          (input[i + inputOffset] * i + pMidBuffer[i] * itemp) / overlapLength;
    }
  }
   ...
   *
   * <p>The best position is determined as the position where the two overlapped sample sequences
   * are 'most alike', in terms of the highest cross-correlation value over the overlapping period
   *
  ...
  /**
   * Slopes the amplitude of the 'midBuffer' samples so that cross correlation is faster to
   * calculate. Why is this faster?
   */
  ...
  public void process(AudioEvent audioEvent) {
    ...
    if (audioEvent.getSampleRate() != sampleRate) {
      newParameters = Parameters.slowdownDefaults(tempo, audioEvent.getSampleRate());
      applyNewParameters();
    }

    for (int channel = 1; channel <= audioEvent.getChannelNumber(); channel++) {
      float[] audioFloatBuffer = audioEvent.getFloatBuffer(channel);
      ...
      System.arraycopy(
          audioFloatBuffer, offset + sequenceLength + overlapLength, pMidBuffer, 0, overlapLength);

      assert outputFloatBuffer.length == getOutputBufferSize();

      audioEvent.setFloatBuffer(outputFloatBuffer, channel);
    }
  }

  /**
   * An object to encapsulate some of the parameters for WSOLA, together with a couple of practical
   * helper functions.
   *
   * @author Joren Six
   */
  public static class Parameters {
    ...
    /**
     * @param tempo The tempo change 1.0 means unchanged, 2.0 is + 100% , 0.5 is half of the speed.
     * @param sampleRate The sample rate of the audio 44.1kHz is common.
     * @param newSequenceMs Length of a single processing sequence, in milliseconds. This determines
     *     to how long sequences the original sound is chopped in the time-stretch algorithm.
     *     <p>The larger this value is, the lesser sequences are used in processing. In principle a
     *     bigger value sounds better when slowing down tempo, but worse when increasing tempo and
     *     vice versa.
     *     <p>Increasing this value reduces computational burden & vice versa.
     * @param newSeekWindowMs Seeking window length in milliseconds for algorithm that finds the
     *     best possible overlapping location. This determines from how wide window the algorithm
     *     may look for an optimal joining location when mixing the sound sequences back together.
     *     <p>The bigger this window setting is, the higher the possibility to find a better mixing
     *     position will become, but at the same time large values may cause a "drifting" artifact
     *     because consequent sequences will be taken at more uneven intervals.
     *     <p>If there's a disturbing artifact that sounds as if a constant frequency was drifting
     *     around, try reducing this setting.
     *     <p>Increasing this value increases computational burden & vice versa.
     * @param newOverlapMs Overlap length in milliseconds. When the chopped sound sequences are
     *     mixed back together, to form a continuous sound stream, this parameter defines over how
     *     long period the two consecutive sequences are let to overlap each other.
     *     <p>This shouldn't be that critical parameter. If you reduce the DEFAULT_SEQUENCE_MS
     *     setting by a large amount, you might wish to try a smaller value on this.
     *     <p>Increasing this value increases computational burden & vice versa.
     */
    public Parameters(
        double tempo, double sampleRate, int newSequenceMs, int newSeekWindowMs, int newOverlapMs) {
      this.tempo = tempo;
    ...
    public Parameters speechDefaults(double tempo, double sampleRate) {
      int sequenceMs = 40;
      int seekWindowMs = 15;
      int overlapMs = 12;
    ...
    public Parameters musicDefaults(double tempo, double sampleRate) {
      int sequenceMs = 82;
      int seekWindowMs = 28;
      int overlapMs = 12;
      return new Parameters(tempo, sampleRate, sequenceMs, seekWindowMs, overlapMs);
    ...
    public static Parameters slowdownDefaults(double tempo) {
      int sequenceMs = 100;
      int seekWindowMs = 35;
      int overlapMs = 20;
      return new Parameters(tempo, 44100, sequenceMs, seekWindowMs, overlapMs);
    }
    ...
    public Parameters automaticDefaults(double tempo, double sampleRate) {
